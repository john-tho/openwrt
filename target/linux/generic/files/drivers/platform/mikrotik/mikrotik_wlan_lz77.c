...
#define MIKRO_LZ77_MAX_ENCODED 0x1000
#define MIKRO_LZ77_MAX_DECODED 0x40000

/* examples have all decompressed to start with DRE\x00 */
#define MIKRO_LZ77_EXPECTED_OUT 0x44524500
 ...
 */
static unsigned int mikrotik_wlan_lz77_get_bit(
 ...
 */
static unsigned char mikrotik_wlan_lz77_get_byte(
		const unsigned char *in,
		unsigned int in_offset_bit)
{
	unsigned char buf = 0;
		...
		unsigned int size)
{
	...
	size += pos;
	*bits_used = 0;
	pr_debug(MIKRO_LZ77 "decode_count inbit: %i, start shift:%i, initial count:%i\n",
		...
		size_t *out_len)
{
	...
	if (in_len > MIKRO_LZ77_MAX_ENCODED || (in_len * 8) > UINT_MAX) {
	...
	while (true) {
		...
		pr_debug(MIKRO_LZ77 "inbit:0x%x->outbyte:0x%x",
		...
		case INSTR_LONG:
			if (opcode->offset == 0) {
				...
				if (opcode->length == 0xc &&
					...
					if (!(*(unsigned int *)out == MIKRO_LZ77_EXPECTED_OUT)) {
						*out_len = output_ptr - out;
						pr_debug(MIKRO_LZ77 "lz77 decompressed from %zu to %zu\n",
								in_len, *out_len);
						return 0;
					} else {
						pr_err(MIKRO_LZ77 "lz77 decompressed: unexpected output\n");
						return -EINVAL;
					}
				}
