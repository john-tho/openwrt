 ...
 */
static const u8 mikrotik_wlan_lzor_prefix[] = {
 ...
 * that magic number is a payload that must be appended to the
 * mikrotik_wlan_lzor_prefix the resulting blob is LZO-compressed.
 * In the LZO decompression result,
...
{
	u8 *tempbuf;
	size_t prefix_len, templen, lzo_len;
	int ret;

	prefix_len = sizeof(mikrotik_wlan_lzor_prefix);
	lzo_len = inlen + prefix_len;
	if (lzo_len > *outlen)
		return -EFBIG;
	...
	memcpy(tempbuf, mikrotik_wlan_lzor_prefix, prefix_len);
	memcpy(tempbuf + prefix_len, inbuf, inlen);

	/* LZO-decompress lzo_len bytes of compressed Wi-Fi data
	 * (prepended with constant prefix) into outbuf
	 */
	ret = lzo1x_decompress_safe(tempbuf, lzo_len, outbuf, outlen);

	kfree(tempbuf);
	return ret;
}
EXPORT_SYMBOL_GPL(mikrotik_wlan_lzor_prefixed_decompress);
...
EXPORT_SYMBOL_GPL(mikrotik_wlan_rle_decompress);

MODULE_LICENSE("GPL v2");
MODULE_DESCRIPTION("MikroTik Wi-Fi caldata decompression library");
